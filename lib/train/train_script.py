...
from lib.train.actors import OSTrackActor, OSTrackCMPActor
# for import modules
import importlib
...
def run(settings):
    ...
    elif settings.script_name == "ostrackcmp":
        net = build_ostrackcmp(cfg)
    else:
    ...
    if settings.script_name == "ostrack":
        ...
        objective = {'giou': giou_loss, 'l1': l1_loss, 'focal': focal_loss, 'cls': BCEWithLogitsLoss()}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'focal': 1., 'cls': 1.0}
        actor = OSTrackCMPActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
