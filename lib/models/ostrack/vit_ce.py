...
class CEBlock(nn.Module):
    ...
    def forward(self, xz, global_index_z, global_index_x, mask=None, ce_template_mask=None, keep_ratio_search=None, num_template=1):
        x_attn, attn = self.attn(self.norm1(xz), mask, True)
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            ...
            xz, global_index_x, removed_index_x = candidate_elimination(
                tokens=xz, attn=attn, lens_z=lens_z, keep_ratio=keep_ratio_search,
...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, template_list, search_list, template_anno_list,
        ...
        for i, blk in enumerate(self.blocks):   # TODO: 等会继续从这里开始
            xz, global_index_z, global_index_x, removed_index_x, attn = \
                blk(xz, global_index_z, global_index_x, mask_x, ce_template_mask, ce_keep_rate, len(template_list))
